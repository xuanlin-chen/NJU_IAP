...
def analyze_article(content):
    ...
    prompt_filter = f"""内容分析：提取关键要素（核心内容/时间/对象）
有用性判断：
这里文章是否有用，主要是由本科生在阅读完这篇文章是否会产生明确的需要完成的任务来决定，该文章是否能起到类似于行动指南的效果，如果能，则判断为有用
○ 是否具有行动指引功能（如课程通知、考试安排、选课信息、活动报名等明确要求学生采取行动的内容）
○ 是否涉及生活必需（宿舍调整、校园活动、安全提醒等需要学生知晓并应对的内容）
○ 是否提供发展机会（竞赛通知、实习机会、学术讲座等需要学生把握的机遇）
学生参与度判断（若有且对象为本科生，需进一步判断）：
○ 是否要求学生采取具体行动（如报名、准备材料、参加活动等）
○ 是否影响学业进度或生活安排（如考试时间、课程调整、活动时间冲突等）
○ 是否涉及资格/机会的获取条件（如竞赛资格、实习要求、选课条件等）
需排除的内容（直接判断为“无用”）：
对已经完成的活动的总结或者记录，且对本科生后续行程没有具体的安排
○ 纯工作部署类会议报道
○ 未落地的规划方案讨论
○ 无具体实施路径的政策宣导
○ 无本科生参与机制的教研活动
○ 仅对已结束活动进行总结且对本科生无后续影响的内容（若总结内容能为本科生后续行动提供明确指引或参考，则不在此列）
思考判断：若将文章判断为“无用”，请再次谨慎思考是否真的“无用”，以保证不遗漏信息
 最终结论：仅输出"有用"或者"无用"
